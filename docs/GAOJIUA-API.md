...
**这个云盘里几乎每份文件都存在两份内容完全相同的副本**，区别只在文件名：

```
广东卷/地理/广东地理.pdf
广东卷/地理/广东地理(13-28-40-6833).pdf          ← 与上面完全同一内容
```

括号里的 `(13-28-40-6833)` 看起来是上传时自动加的编号。接口把它们返回成**两个独立条目**，各有自己的 `id`，所以按正常逻辑会各下一份。

实测数据（一次真实下载，共 279 个文件）：
...
| 不同内容（按 MD5 去重后） | **139** |
| 重复副本 | 140 个 |
| 冗余占用 | 158.0 MB（占总量的 50%） |
| 带括号后缀、且能找到同名对应文件的组合 | 139 对 |
| 其中内容**逐字节完全相同** | **139 对（100%）** |
| 内容不同的 | 0 对 |

验证不只是比对 MD5：对最大的一组（7.36 MB）额外做了逐字节 `Buffer.equals()` 比较，确认完全相同。

### 结论

- **不能靠文件名判断是否重复**，要去重必须比对内容（用 `md5` 字段或实际计算）
- 本工具**默认不去重** —— 要不要留两份、留哪一份是使用者的决定，工具不该替用户丢文件
- 需要清理时用 `scripts/dedupe-files.mjs`：**默认只预览不删除**，仅在确认存在完全相同副本时才动手，
  并优先保留名字干净的那份（不带编号后缀）
