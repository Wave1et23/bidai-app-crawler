...
| `--md5-warn` | MD5 与服务器记录不符时保留文件（默认视为失败，见下方「完整性校验」） |
| `--dry-run` | 只列清单，不下载 |
| `--limit <N>` | 只处理前 N 个文件 |
...
scripts/dedupe-files.mjs    按内容去重（默认只预览不删除）
test/                       本地假站点 + 测试套件
...
这个云盘**会把同一份文件存两份**，一份文件名干净、另一份末尾多一个 `(13-28-40-6833)` 这样的编号，内容完全相同（实测 139 对全部逐字节一致，占总量约 50%）。

工具**默认不去重** —— 要不要留两份、留哪一份应该由你决定，工具不该替你丢文件。需要清理时：

```bash
# 先预览（不会删除任何文件）
node scripts/dedupe-files.mjs "D:\资料\2026真题"

# 确认无误再执行，优先保留文件名干净的那份
node scripts/dedupe-files.mjs "D:\资料\2026真题" --apply
```

它只在**确认存在完全相同的副本**时才删除，删除前会重新校验，校验不过就跳过。

### 完整性校验与 MD5 不符

每个文件都会做两道校验：`Content-Length` 比对 + 与服务端下发的 `md5` 比对。

服务端的 md5 偶尔是**过期的**（实测有文件记录的 size/md5 与实际内容不符，但 `Content-Length` 与实际字节数一致、文件本身合法完整）。这种情况默认按失败处理，因为工具无法区分「元数据过期」和「传输损坏」。

如果确认是前者，可以加 `--md5-warn`：文件会被保留，但**会打印醒目告警并写入 `report.json` 的 `warnings`**，不会被悄悄放过。

```bash
node src/download.mjs --folder "某个文件夹" --md5-warn
```
